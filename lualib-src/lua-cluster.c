...
lpack(lua_State *L) {
	...
	size_t namelen = 0;
	const char *name = lua_tolstring(L, 1, &namelen);
	lua_remove(L,1);//remove addr
	lua_remove(L,1);//remove session
	int sz = 0;
	//	void * msg =  lua_seri(L,&sz);
	_luaseri_pack(L);	
	void * msg =  lua_touserdata(L,-2);
	sz = luaL_checkinteger(L,-1);

	lua_remove(L,-1);//remove sz
	lua_remove(L,-1);//remove msg

	if(msg == NULL){
		return luaL_error(L, "serialize data error!");
	}
...
#else
  while(lua_pushnumber(L,idx),lua_gettable(L, -2),lua_type(L,-1) == LUA_TSTRING){
#endif
		size_t s;
